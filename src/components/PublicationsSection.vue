  ...
  <section id="publications" class="py-16 sm:py-20 bg-gray-50 dark:bg-gray-800">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        <div
          v-for="(pubs, type) in publications"
          :key="type"
          class="space-y-6"
        >
          <h3 class="text-2xl font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-2">
            {{ type }}s
          </h3>
          <ol
            v-if="pubs.length > 0"
          ...
          >
            ...
            >
              <span
                class="absolute flex items-center justify-center w-6 h-6 bg-blue-100 dark:bg-blue-900 rounded-full -start-3 ring-8 ring-gray-50 dark:ring-gray-800"
              >
                <i
                  class="fas fa-file-text text-blue-800 dark:text-blue-300 text-xs"
                ></i>
              </span>
              <h4
              ...
              >
                ...
                <span
                  v-if="index === 0"
                  class="bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-300 text-sm font-medium me-2 px-2.5 py-0.5 rounded-sm ms-3"
                  >Latest</span
                >
              </h4>
              <time
              ...
              <div class="flex gap-3 flex-wrap">
                ...
                <button
                  ...
                  class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 hover:text-blue-700 dark:hover:text-blue-400 focus:z-10 focus:ring-4 focus:outline-none focus:ring-gray-100 dark:focus:ring-gray-700 cursor-pointer"
                >
              ...
              >
                <button
                  @click="copyCitation(pub.cite, pub.id)"
                  ...
                  :class="{
                    'bg-green-600 hover:bg-green-600': copySuccess[pub.id],
                  }"
                  title="Copy to clipboard"
                >
                  <i
                    :class="copySuccess[pub.id] ? 'fas fa-check' : 'fas fa-copy'"
                  ></i>
                  <span>{{ copySuccess[pub.id] ? "Copied!" : "Copy" }}</span>
                </button>
                <pre
                  class="bg-gray-900 dark:bg-gray-950 text-gray-100 p-4 rounded-lg overflow-x-auto text-xs citation-code"
                  tabindex="0"
                ><code>{{ pub.cite }}</code></pre>
              </div>
            </li>
          </ol>
        </div>
        <p v-if="Object.keys(publications).length === 0" class="text-center text-gray-600 dark:text-gray-400">
  ...
  methods: {
    async loadPublications() {
      try {
        ...
        this.publications = groupedPublications
